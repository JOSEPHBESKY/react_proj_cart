...
export default function Cart() {
    ...
    const remove = (items) => {
        let dismiss = state.Cartidems.splice(0,1)
        dispatch({
          type: "rmvItem",
          payload: dismiss,
        });
...
<div className='display'>   <Box sx={{ flexGrow: 1 }}>
...
<div className='container'>
    {
            ...
            <div key={index} className='cars'><h1>{products.name}</h1><img src={products.img} alt="" className="image"></img><p className='price'>{products.price}</p><p className='qty'>  {products.qty}</p>
            
            <button className="remove"  onClick={() => remove(products)}>x</button>
           
                <button onClick={() => increase( products)} className="plus">+</button>
                <span>{state.qty[products.id]}</span>
                <button onClick={() => decrease(products)} className="minus">-</button></div>
        ))
    }
    </div>
    </div>
